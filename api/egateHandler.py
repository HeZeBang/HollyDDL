...
def getBB(sess:requests.Session):
    response = sess.get("https://elearning.shanghaitech.edu.cn:8443/webapps/calendar/calendarData/allCourseEvents?start="+str(int(datetime.now().timestamp())*1000),
                headers=headers)
    ...
    for item in response:
        ...
        url = "https://elearning.shanghaitech.edu.cn:8443/webapps/calendar/launch/attempt/_blackboard.platform.gradebook2.GradableItem-" + item['itemSourceId']
        data.append({
            'title': title,
            'course': course,
            'due': due,
    ...
    response = sess.get("https://elearning.shanghaitech.edu.cn:8443/webapps/calendar/calendarData/pastDueEvents",
